...
func TestIPv4Sanity(t *testing.T) {
	...
	tests := []struct {
		...
		// The next two tests check that we are not rejecting an incoming packet
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			// We expect a packet. Is it large enough to safely validate?
			if got, want := reply.Pkt.Size(), header.IPv4MinimumSize+header.ICMPv4MinimumSize; got < want {
				t.Fatalf("got small reply packet of size %d, want >= %d", got, want)
			}
			...
			case header.ICMPv4DstUnreachable:
				checker.IPv4(t, replyIPHeader,
					...
					checker.FullLen(uint16(header.IPv4MinimumSize+header.ICMPv4MinimumSize+requestPkt.Size())),
					checker.HeaderLen(ipHeaderLength),
					checker.ICMPv4(
			...
			case header.ICMPv4EchoReply:
				checker.IPv4(t, replyIPHeader,
					...
					checker.DstAddr(remoteIPv4Addr),
					checker.HeaderLen(ipHeaderLength),
					checker.FullLen(uint16(requestPkt.Size())),
					checker.ICMPv4(
						checker.ICMPv4Code(test.ICMPCode),
